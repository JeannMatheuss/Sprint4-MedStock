...
class MaterialServiceTest {
    private final MaterialDAO dao = mock(MaterialDAO.class);
    private final MaterialService service = new MaterialService(dao);

    @Test
    void criar_valido() throws SQLException {
        Material m = new Material(null,"Soro",10.0,"ml",5.0);
        doAnswer(inv -> { m.setId(1L); return null; }).when(dao).save(m);
        Material created = service.criar(m);
